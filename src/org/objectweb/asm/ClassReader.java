...
public class ClassReader
{
    ...
    private int readMethod(final ClassVisitor classVisitor, final Context context, final int methodInfoOffset) {
        ...
        if (methodVisitor instanceof MethodWriter) {
            ...
            if (methodWriter.canCopyMethodAttributes(this, methodInfoOffset, currentOffset - methodInfoOffset, synthetic, (context.currentMethodAccessFlags & 0x20000) != 0x0, this.readUnsignedShort(methodInfoOffset + 4), signatureIndex, exceptionsOffset)) {
                return currentOffset;
            }
        }
    ...
    private void readCode(final MethodVisitor methodVisitor, final Context context, final int codeOffset) {
        ...
        if (stackMapFrameOffset != 0) {
            ...
            for (int offset = stackMapFrameOffset; offset < stackMapTableEndOffset - 2; ++offset) {
                if (classFileBuffer[offset] == 8) {
                    final int potentialBytecodeOffset = this.readUnsignedShort(offset + 1);
                    if (potentialBytecodeOffset >= 0 && potentialBytecodeOffset < codeLength && (classFileBuffer[bytecodeStartOffset + potentialBytecodeOffset] & 0xFF) == 0xBB) {
        ...
        while (currentOffset < bytecodeEndOffset) {
            ...
            int opcode2 = classFileBuffer[currentOffset] & 0xFF;
            switch (opcode2) {
                case 0:
                case 1:
                case 2:
                ...
                case 196: {
                    opcode2 = (classFileBuffer[currentOffset + 1] & 0xFF);
                    if (opcode2 == 132) {
                ...
                case 58:
                case 169: {
                    methodVisitor.visitVarInsn(opcode2, classFileBuffer[currentOffset + 1] & 0xFF);
                    currentOffset += 2;
                    break;
                }
                case 16:
                case 188: {
                    methodVisitor.visitIntInsn(opcode2, classFileBuffer[currentOffset + 1]);
                    currentOffset += 2;
                    break;
                }
                case 17: {
                ...
                case 185: {
                    ...
                    if (opcode2 < 182) {
                        ...
                        final boolean isInterface = classFileBuffer[cpInfoOffset - 1] == 11;
    ...
    private int[] readTypeAnnotations(final MethodVisitor methodVisitor, final Context context, final int runtimeTypeAnnotationsOffset, final boolean visible) {
        ...
        for (int i = 0; i < typeAnnotationsOffsets.length; ++i) {
            ...
            if (targetType >>> 24 == 66) {
                final TypePath path = (pathLength == 0) ? null : new TypePath(this.b, currentOffset);
    ...
    private void readParameterAnnotations(final MethodVisitor methodVisitor, final Context context, final int runtimeParameterAnnotationsOffset, final boolean visible) {
        ...
        final int numParameters = this.b[currentOffset++] & 0xFF;
    ...
    private int readStackMapFrame(final int stackMapFrameOffset, final boolean compressed, final boolean expand, final Context context) {
        ...
        int frameType;
        if (compressed) {
            frameType = (this.b[currentOffset++] & 0xFF);
        }
        else {
            frameType = 255;
            context.currentFrameOffset = -1;
    ...
    private int readVerificationTypeInfo(final int verificationTypeInfoOffset, final Object[] frame, final int index, final char[] charBuffer, final Label[] labels) {
        ...
        final int tag = this.b[currentOffset++] & 0xFF;
        switch (tag) {
            case 0: {
    ...
    private String readUtf(final int utfOffset, final int utfLength, final char[] charBuffer) {
        ...
        while (currentOffset < endOffset) {
            final int currentByte = classFileBuffer[currentOffset++];
