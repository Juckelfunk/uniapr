...
public class MXParser implements XmlPullParser
{
    ...
    public void defineEntityReplacementText(final String entityName, String replacementText) throws XmlPullParserException {
        if (!replacementText.startsWith("&#") && this.entityName != null && replacementText.length() > 1) {
            final String tmp = replacementText.substring(1, replacementText.length() - 1);
    ...
    protected char[] parseEntityRef() throws XmlPullParserException, IOException {
        ...
        if (ch == '#') {
            ...
            this.posEnd = this.pos - 1;
            if (sb.length() > 0) {
                final char[] tmp = toChars(Integer.parseInt(sb.toString(), 16));
                this.charRefOneCharBuf = tmp;
                if (this.tokenize) {
    ...
    private static boolean isValidCodePoint(final int codePoint) {
        return 0 <= codePoint && 1114111 >= codePoint;
    }
    
    private static boolean isSupplementaryCodePoint(final int codePoint) {
    ...
    public static char[] toChars(final int codePoint) {
        ...
        if (isSupplementaryCodePoint(codePoint)) {
            final int cpPrime = codePoint - 65536;
            final int high = 0xD800 | (cpPrime >> 10 & 0x3FF);
            final int low = 0xDC00 | (cpPrime & 0x3FF);
